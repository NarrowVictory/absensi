...
<?php
	...
	if(empty($nama))
	{
		...
		if(!empty($tgl_lahir)){
			if($sql=mysqli_query($link, "update guru set tgl_lahir='$tgl_lahir' where id_guru ='$nim'")){
		...
		if(!empty($foto)){
			if($sql=mysqli_query($link, "update guru set foto_profil='$foto' where id_guru ='$nim'") && move_uploaded_file($_FILES["file"]["tmp_name"], $target) ){
		...
		echo '<META HTTP-EQUIV="Refresh" Content="0; URL=pgturanAkun.php">';
