...
<?php
	...
	if( empty($telepon_guru)  || empty($nama_guru) || empty($email_guru) || empty($username_guru) )
	{
		...
		if(!empty($tanggallahir_guru)){
			if($sql=mysqli_query($link, "update guru set tgl_lahir='$tanggallahir_guru' where id_guru='$nip_guru'")){
		...
		if(!empty($password_guru)){
			if($sql=mysqli_query($link, "update guru set password=md5('$password_guru') where id_guru='$nip_guru'")){
		...
		if(!empty($foto_guru)){
			if($sql=mysqli_query($link, "update guru set foto_profil='$foto_guru' where id_guru='$nip_guru'") && move_uploaded_file($_FILES['file']['tmp_name'],$target) ){
